...
{
    // yaycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespanRe = 1 * 30; // 30 seconds
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    if (vtx[0].GetValueOut() > GetBlockValue(pindex->nHeight, nFees, pindex->pprev->nBits))
        return state.DoS(100, error("ConnectBlock() : coinbase pays too much (actual=%"PRI64d" vs limit=%"PRI64d")", vtx[0].GetValueOut(), GetBlockValue(pindex->nHeight, nFees, pindex->pprev->nBits)));
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // yaycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // yaycoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x09c6cb60f9b90dc592d24e5b62a12c3f62b1692f19c815354261df77aab15240");
    }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // yaycoin: increase each by adding 2 to bitcoin's value.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    int64 nFees = 0;
    {
        ...
        pblock->vtx[0].vout[0].nValue = GetBlockValue(pindexPrev->nHeight+1, nFees, pindexPrev->nBits);
